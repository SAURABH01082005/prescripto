    ...
    const options = {
      ...
      handler:async (response) => {
        ...
        try{
          ...
          const {data} = await axios.post(backendUrl + '/api/user/verifyRazorpay',response,{headers:{token:token}})
          if(data.success){
            getUserAppointments()
        ...
        {appointments.map((item,index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col justify-end'>
              {!item.cancelled && item.payment && !item.isCompleted && <button className='sm:min-w-48 py-2 border text-stone-500 rounded bg-[#E9ECFF]'>Paid</button>}
